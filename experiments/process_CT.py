...
from Data.ATNet.UNet3d import ATNet
import glob
import torch
...
ct_processed_path = "../Data/processed/CT_gradient"
...
for i in tqdm(range(N)):
   ...
   # this is the original ct data 
    ...
    torch.save(ct_processed,osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'.pt'))

    # ct.clear_data()
    ct.save(osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'.vti'))

    print('create model')
    # define model
    model = ATNet(1, 2).cuda()
    # obtain the gredient
    image_processed=ct_processed.cpu()[0,0,:,:,:]

    # this is calcualting the orignal raw signal gradient ######################################################
    print('calculate gradient ')
    grad_x, grad_y, grad_z, grad_mag, grad_dir=model.gradient_sobel(image)
    grad2_x, grad2_y, grad2_z, grad2_mag, grad2_dir=model.gradient_sobel(grad_mag)
    ...
    ct.clear_data()
    ct.point_data['grads'] = np.transpose(np.asarray(grad_mag),(2,1,0)).reshape(-1)
    ct.save(osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'gradient.vti'))
    # ct.point_data['grads2'] = np.transpose(np.asarray(grad2_mag),(2,1,0)).reshape(-1)
    ############################################################################################################
    
    # this is masking the raw ct data using the label######################################################
    print('storing the segmentation mask')
    temp = ct.point_data['ImageScalars'].copy()
    temp[np.transpose(npy,(2,1,0)).flatten()<0.5] = 0.
    ct.clear_data()
    ct.point_data['ImageScalars'] = temp
    print('saving the ct data to folders')
    ct.save(osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'_masked.vti'))


    ct.clear_data()
    ct.point_data['ImageScalars'] = np.transpose(npy,(2,1,0)).flatten()
    ct.save(osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'_segmented.vti'))
    ############################################################################################################

    # gradient of the masked signal ############################################################################
    print(temp.shape, image_processed.shape)
    grad_x, grad_y, grad_z, grad_mag, grad_dir=model.gradient_sobel(torch.tensor(np.transpose(temp.reshape(ct_shape_inv),(2,1,0)),dtype = torch.float32))
    grads = torch.stack([grad_x, grad_y, grad_z, grad_mag])
    ct.clear_data()
    ct.point_data['gradient'] = np.transpose(np.asarray(grad_mag),(2,1,0)).reshape(-1)
    ct.save(osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'processed_clamped_gradient.vti'))
    ###########################################################################################################



    # # gradient of the preprocessed signal (thresholded and normed) ############################################################################
    grad_x, grad_y, grad_z, grad_mag, grad_dir=model.gradient_sobel(image_processed)
    # grad2_x, grad2_y, grad2_z, grad2_mag, grad2_dir=model.gradient_sobel(grad_mag)
    ...
    ct.point_data['processed_grads'] = np.transpose(np.asarray(grad_mag),(2,1,0)).reshape(-1)
    # ct.point_data['processed_grads2'] = np.transpose(np.asarray(grad2_mag),(2,1,0)).reshape(-1)
    ct.save(osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'processed_gradient.vti'))
    torch.save(grads,osj(postprocess_path,'ct_gradients_new',file_name_list[i][:-4]+'processed_gradient.pt'))
    ###########################################################################################################


    grad_x, grad_y, grad_z, grad_mag, grad_dir=model.gradient_sobel(image_norm)
    grad2_x, grad2_y, grad2_z, grad2_mag, grad2_dir=model.gradient_sobel(grad_mag)
    grads = torch.stack([grad_x, grad_y, grad_z, grad_mag])
    ...
    grads_grads2 = torch.stack([grads,grads2])
    ct.clear_data()
    ct.point_data['norm_imagescalar'] = np.transpose(np.asarray(image_norm),(2,1,0)).reshape(-1)
    ct.point_data['norm_grads'] = np.transpose(np.asarray(grad_mag),(2,1,0)).reshape(-1)
    ct.point_data['norm_grads2'] = np.transpose(np.asarray(grad2_mag),(2,1,0)).reshape(-1)
    ct.save(osj(postprocess_path,'ct_gradients',file_name_list[i][:-4]+'norm.vti'))

    ct.clear_data()
    ct.save(osj(postprocess_path,'ct_gradients',file_name_list[i][:-4]+'_grads.vti'))

    ct.clear_data()
    ct.save(osj(postprocess_path,'ct_gradients',file_name_list[i][:-4]+'_grads2.vti'))

    ct.clear_data()
    ct.point_data['prediction'] = np.transpose(npy,(2,1,0)).reshape(-1)
    ct.save(osj(postprocess_path,'ct_gradients',file_name_list[i][:-4]+'_prediction.vti'))
    ...
    # mesh_phy.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_mc.vtp'))
    # mesh_phy.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_mc.ply'))
    mesh_phy.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_filtered_0.5_mc.vtp'))
    mesh_phy.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_filtered_0.5_mc.ply'))

    # remesh the mc using pyacvd
    remesh = ACVD(mesh_phy, keep_ratio = 1)
    # remesh.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_mc_remesh.vtp'))
    # remesh.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_mc_remesh.ply'))
    remesh.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_filtered_0.5_mc_remesh.vtp'))
    remesh.save(osj(postprocess_path,'surf_mc',file_name_list[i][:-4]+'surf_filtered_0.5_mc_remesh.ply'))
